...
export function InviteBar({ joinCode, poolName }: { joinCode: string; poolName: string }) {
    ...
    <div className="px-4 pb-4">
      <div className="flex items-center gap-2 rounded-2xl border border-border bg-surface px-4 py-3">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[10px] text-textMuted uppercase tracking-widest font-semibold">Código</span>
          <span className="font-display text-base font-extrabold tracking-[0.15em] text-white">{joinCode}</span>
        </div>
        ...
        <button
          onClick={copy}
          className={[
            "flex items-center gap-1.5 h-9 px-3 rounded-xl text-xs font-semibold border transition-all shrink-0",
            copied
              ? "bg-brand-green border-brand-green text-black"
              : "bg-brand-greenDim border-brand-green/60 text-brand-green",
          ].join(" ")}
        >
          {copied ? (
            <>
        ...
        <a
          ...
          className="w-9 h-9 rounded-xl bg-[#25D366] flex items-center justify-center shrink-0"
        >
